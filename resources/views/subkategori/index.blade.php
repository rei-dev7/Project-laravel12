    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-200 p-8">
        ...
        <div class="flex items-center justify-between mb-8 px-5">
            <h2 class="text-4xl font-extrabold text-gray-900 flex items-center space-x-3">
                {{-- Menggunakan Ikon Tags --}}
                <i class="bi bi-tags-fill text-indigo-600 text-4xl"></i>
                <span>Data Master Sub Kategori</span>
            </h2>
        ...
        <div class="flex items-center justify-between mb-6 px-5">
            <div class="flex items-center bg-white px-5 py-2 rounded-xl border border-gray-300 shadow">
                ...
                <span class="text-gray-700">
                    Total Data: <span class="font-bold text-indigo-700">{{ $subKategori->count() }}</span>
                </span>
            </div>
        </div>
            ...
            <table class="w-full table-auto">
                ...
                <thead class="bg-indigo-600 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Nama Sub Kategori</th>
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Kategori Induk</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @forelse($subKategori as $item)
                        <tr class="hover:bg-indigo-50 transition">
                            ...
                            {{-- Menampilkan Nama Kategori Induk --}}
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center space-x-3">
                                    ...
                                    <a href="{{ route('subkategori.edit', $item->id_sub_kategori) }}"
                                       class="px-4 py-2 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition font-medium">
                                        Edit
                                    </a>

                                    <form action="{{ route('subkategori.destroy', $item->id_sub_kategori) }}"
                                          method="POST"
                                          onsubmit="return confirm('Hapus sub kategori {{ $item->nama_sub_kategori }}? Tindakan ini tidak dapat dibatalkan.')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-8 text-gray-500 bg-gray-50">
                                ...
                                <div class="mt-2 text-lg">Belum ada data sub kategori yang terdaftar.</div>
                            </td>
                        </tr>
                    @endforelse
