...
<div class="w-full px-6 py-6">
    ...
    {{-- CARD WRAPPER --}}
    <div class="bg-white px-4 rounded-3xl shadow-xl border border-gray-200 max-w-xl mx-auto">

        {{-- HEADER --}}
        <div class="mb-8 py-4 border-b border-gray-100">
            <h2 class="text-3xl font-extrabold text-gray-900 flex items-center space-x-3">
                <i class="bi bi-tags text-indigo-600 text-4xl"></i>
                <span>Form Tambah Sub Kategori</span>
            </h2>

            <p class="text-gray-500 mt-2 ml-10">
                Isi data sub kategori baru, pastikan terhubung dengan kategori induk yang benar.
            </p>
        </div>

        {{-- FORM --}}
        <form action="{{ route('subkategori.store') }}" method="POST" class="mt-4 py-4">
            @csrf
