        ...
        <form action="{{ route('barangmasuk.update', $barangMasuk->id_masuk) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                    ...
                    <select name="id_item" class="w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 p-3 bg-gray-50" required>
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id_item }}" 
                                {{ old('id_item', $barangMasuk->id_item) == $item->id_item ? 'selected' : '' }}>
                    ...
                    <select name="id_supplier" class="w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 p-3 bg-gray-50" required>
                        ...
                        @foreach($suppliers as $sup)
                            <option value="{{ $sup->id_supplier }}" 
                                {{ old('id_supplier', $barangMasuk->id_supplier) == $sup->id_supplier ? 'selected' : '' }}>
                                {{ $sup->nama_supplier }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number"
                           name="jumlah_stok"
                           value="{{ old('jumlah_stok', $barangMasuk->jumlah_stok) }}"
                           min="1"
